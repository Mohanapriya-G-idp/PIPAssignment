...
export abstract class BaseEyes {
  ...
  seesClassVisible(domClass: string) {
    cy.get(`.${domClass}`).should('be.visible');
    return this;
  }
...
export class BaseHands {
  ...
  clickOnDomElementByAttribute(attributeName: string, attributeValue: string) {
    cy.get(`[${attributeName}="${attributeValue}"]`).click();
    return this;
}
